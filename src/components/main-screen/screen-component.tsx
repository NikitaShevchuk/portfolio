...
const Screen = ({ className, link, title, textToCopy, image, order, showButton, Body, subtitle }: Props) => {
          ...
          <div className='screen__title-subtitle __title-subtitle'>
            ...
            {textToCopy &&
              textToCopy.map((textItem) => (
                <div key={textItem} onClick={() => copyText(textItem)} className='mt hover-underline'>
                  {textItem}
                </div>
              ))}
          </div>
